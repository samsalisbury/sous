...
func (self *ExecutorID) Absorb(other swaggering.DTO) error {
	if like, ok := other.(*ExecutorID); ok {
		*self = *like
		return nil
	}
	return fmt.Errorf("A ExecutorID cannot copy the values from %#v", other)
}
...
func (self *ExecutorID) SetField(name string, value interface{}) error {
	...
	case "defaultInstanceForType", "DefaultInstanceForType":
		...
		if ok {
			...
			return fmt.Errorf("Field defaultInstanceForType/DefaultInstanceForType: value %v(%T) couldn't be cast to type *ExecutorID", value, value)
		}
...
func (self *ExecutorIDList) Absorb(other swaggering.DTO) error {
	if like, ok := other.(*ExecutorIDList); ok {
		*self = *like
		return nil
	}
	return fmt.Errorf("A ExecutorIDList cannot copy the values from %#v", other)
}
