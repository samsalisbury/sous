...
func (self *SlaveID) Absorb(other swaggering.DTO) error {
	if like, ok := other.(*SlaveID); ok {
		*self = *like
		return nil
	}
	return fmt.Errorf("A SlaveID cannot copy the values from %#v", other)
}
...
func (self *SlaveID) SetField(name string, value interface{}) error {
	...
	case "defaultInstanceForType", "DefaultInstanceForType":
		...
		if ok {
			...
			return fmt.Errorf("Field defaultInstanceForType/DefaultInstanceForType: value %v(%T) couldn't be cast to type *SlaveID", value, value)
		}
...
func (self *SlaveIDList) Absorb(other swaggering.DTO) error {
	if like, ok := other.(*SlaveIDList); ok {
		*self = *like
		return nil
	}
	return fmt.Errorf("A SlaveIDList cannot copy the values from %#v", other)
}
