...
func (ar *AutoResolver) write(f func()) {
	...
	messages.ReportLogFieldsMessage("Waiting to lock autoresolve for write", logging.ExtraDebug1Level, ar.LogSink, name, uuid)
	ar.Lock()
	messages.ReportLogFieldsMessage("Locked autoresolve for write", logging.ExtraDebug1Level, ar.LogSink, name, uuid)
	defer func() {
		ar.Unlock()
		messages.ReportLogFieldsMessage("Unlocked autoresolver", logging.ExtraDebug1Level, ar.LogSink, name, uuid)
	}()
	f()
}
