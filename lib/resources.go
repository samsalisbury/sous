...
func (r Resources) Cpus() float64 {
	...
	if err != nil {
		cpus = 0.1
		if present {
			reportResourceMessage(fmt.Sprintf("Could not parse value: '%s' for cpus as a float, using default: %f", cpuStr, cpus), r)
		} else {
			reportDebugResourceMessage(fmt.Sprintf("Using default value for cpus: %f.", cpus), r)
		}
	}
	return cpus
}
...
func (r Resources) Memory() float64 {
	...
	if err != nil {
		memory = 100
		if present {
			reportResourceMessage(fmt.Sprintf("Could not parse value: '%s' for memory as an int, using default: %f", memStr, memory), r)
		} else {
			reportDebugResourceMessage(fmt.Sprintf("Using default value for memory: %f.", memory), r)
		}
	}
	return memory
}
...
func (r Resources) Ports() int32 {
	...
	if err != nil {
		ports = 1
		if present {
			reportResourceMessage(fmt.Sprintf("Could not parse value: '%s' for ports as a int, using default: %d", portStr, ports), r)
		} else {
			reportDebugResourceMessage(fmt.Sprintf("Using default value for ports: %d", ports), r)
		}
	}
	return int32(ports)
}
...
func (r Resources) Equal(o Resources) bool {
	reportDebugResourceMessage(fmt.Sprintf("Comparing resources: %+ v ?= %+ v", r, o), r)
	if len(r) != len(o) {
		reportDebugResourceMessage("Lengths differ", r)
		return false
	}

	if r.Ports() != o.Ports() {
		reportDebugResourceMessage("Ports differ", r)
		return false
	}

	if math.Abs(r.Cpus()-o.Cpus()) > 0.001 {
		reportDebugResourceMessage("Cpus differ", r)
		return false
	}

	if math.Abs(r.Memory()-o.Memory()) > 0.001 {
		reportDebugResourceMessage("Memory differ", r)
		return false
	}
...
func reportDebugResourceMessage(msg string, r Resources) {
	reportResourceMessage(msg, r, true)
}

func reportResourceMessage(msg string, r Resources, debug ...bool) {
	log := *(logging.SilentLogSet().Child("Resources").(*logging.LogSet))
